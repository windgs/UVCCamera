...
public final class MainActivity extends BaseActivity implements CameraDialog.CameraDialogParent {
	...
	/**
	 * 在私有线程上依次执行摄像机相关方法的处理器
	 * 可以创建自己的线程，通过一个Handler对象和应用的主线程通信。
	 * 里面包含了自己的线程CameraThread：mWeakThread
	 */
	...
	* 如果UI线程blocked的时间太长（大约超过5秒），用户就会看到ANR（application not responding）的对话框
	...
	*  */
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		//设置mUSBMonitor的上级指针this：MainActivity
	...
	private void startPreview()
	{
		...
		//开始预览图像 ，执行uvc_start_streaming_bandwidth
	...
	//相机连接,成员变量会通过onCreate透传到USBMonitor构造函数中
	private final OnDeviceConnectListener mOnDeviceConnectListener = new OnDeviceConnectListener() {
		...
		*/
		...
		public void onConnect(final UsbDevice device, final UsbControlBlock ctrlBlock, final boolean createNew)
		{
			...
			//主线程(mainActivity)中打开相机，ctrlBlock透传=>sendMessage(MSG_OPEN)=>thread.handleOpen=>new UVCCamera=>camera.open
			mCameraHandler.open(ctrlBlock);

			//开启流，开始预览图像 ，执行uvc_start_streaming_bandwidth
			startPreview();
...
//================================================================================
	...
	private final Runnable mUpdateItemsOnUITask = new Runnable()
	{
		...
		public void run()
		{
			...
			mBrightnessButton.setVisibility( checkSupportFlag(UVCCamera.PU_BRIGHTNESS) ? visible_active : View.INVISIBLE);
			mContrastButton.setVisibility( checkSupportFlag(UVCCamera.PU_CONTRAST) ? visible_active : View.INVISIBLE);
		}
	};
	...
	{
		if (DEBUG)
			Log.v(TAG, String.format("showSettings:%08x", mode));
		hideSetting(false);
		if (isActive())
		{
			switch (mode)
	...
	/**
	 * 隐藏配置屏幕
	 * @param fadeOut如果为true，则淡出；如果为false，则立即隐藏
	 */
	...
	private final SeekBar.OnSeekBarChangeListener mOnSeekBarChangeListener = new SeekBar.OnSeekBarChangeListener()
	{
		...
		{
			// 通过触摸寻道栏更改值时切换到onProgressChanged
			// 因为好像不去，所以在这里也要延长隐藏的时间
			runOnUiThread(mSettingHideTask, SETTINGS_HIDE_DELAY_MS);
	...
	//动画效果监听器
	private final ViewAnimationHelper.ViewAnimationListener mViewAnimationListener = new ViewAnimationHelper.ViewAnimationListener()
	{
		@Override
	...
	//					target.setVisibility(View.GONE);
		...
		@Override@NonNull
		public void onAnimationCancel( final Animator animator, @NonNull final View target, final int animationType)
		{
			if (DEBUG) Log.v(TAG, "onAnimationCancel:");
		}
	};
