...
abstract class AbstractUVCCameraHandler extends Handler {
	...
	//打开相机设备，发送MSG_OPEN给主线程
	public void open(final USBMonitor.UsbControlBlock ctrlBlock) {
		checkReleased();
		//发送到message queue 然后由mCameraHandler类执行由分发器(looper)分发下来的message在该handleMessage接口进行处理
	...
	//接收处理器(looper)分发下来的消息(message)，然后进行处理
	@Override
	public void handleMessage(final Message msg) {
		...
		switch (msg.what)
		{
			...
			case MSG_CLOSE:
				thread.handleClose();
				break;
			case MSG_PREVIEW_START:	//开始预览图像 ，执行uvc_start_streaming_bandwidth
				thread.handleStartPreview(msg.obj);
				break;
			case MSG_PREVIEW_STOP:	//停止预览图像
				thread.handleStopPreview();
				break;
			case MSG_CAPTURE_STILL:	//静态预览图像
				thread.handleCaptureStill((String)msg.obj);
				break;
			case MSG_CAPTURE_START:				//开启视频流
				thread.handleStartRecording();
				break;
			case MSG_CAPTURE_STOP:				//停止视频流
	...
	static final class CameraThread extends Thread {
		...
		private int mWidth, mHeight, mPreviewMode; // FRAME_FORMAT_MJPEG;
		 ...
		 */
		CameraThread(final Class<? extends AbstractUVCCameraHandler> clazz, final Activity parent, final CameraViewInterface cameraView,
		...
		private final IFrameCallback mIFrameCallback = new IFrameCallback() {
			...
			{
				if (DEBUG)
					Log.v(TAG_THREAD, "mIFrameCallback->onFrame:");
		...
		//这个run()方法中的语句执行完的时候，这个线程就结束了
		@Override
		public void run() {
			//调用Looper.prepare()检查当前线程中是否有Looper，如果没有则创建,当创建Looper时，它创建了一个消息队列（MessageQueue），用来存放消息（Message）
			Looper.prepare();
			...
			if (handler != null)
			{
				...
				//调用Looper.loop()后Looper开始循环。这样，Looper循环并保持线程存活
				Looper.loop();
