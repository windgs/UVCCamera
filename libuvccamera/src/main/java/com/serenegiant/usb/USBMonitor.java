...
public final class USBMonitor {
	 ...
	 * @param device
	 * @return
	 * @throws 如果您没有SecurityException权限，则抛出SecurityException
	 */
	public UsbControlBlock openDevice(final UsbDevice device) throws SecurityException {
		if (hasPermission(device))
		{
			...
			if (result == null)
			{
				result = new UsbControlBlock(USBMonitor.this, device);    // 在此打开设备
	...
	/**
	 * 生成设备密钥名称以存储每个USB设备的设置。
	 * 由供应商ID，产品ID，设备类，设备子类和设备协议生成
	 * 注意，密钥名称与同类产品相同
	...
	/**
	 * 生成设备密钥名称以存储每个USB设备的设置。
	 * 请注意，useNewAPI=false会产生与同类产品相同的设备密钥
	 * @param device
	 * @param useNewAPI
	 * @return
	 */
...
//		if (DEBUG) Log.v(TAG, "getDeviceKeyName:" + sb.toString());
	...
	/**
	 * 从指定ID的String描述符中获取字符串。 如果无法获取，则返回null
	 * @param connection
	 * @param id
	 * @param languageCount
	 * @param languages
