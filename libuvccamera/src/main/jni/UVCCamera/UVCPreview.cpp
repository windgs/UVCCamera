...
int UVCPreview::startPreview() {
	...
	if (!isRunning()) {
		...
		{
			if (LIKELY(mPreviewWindow)) {
			    LOGW("UVCPreview::startPreview->pthread_create");
...
void *UVCPreview::preview_thread_func(void *vptr_args) {
	...
	if (LIKELY(preview)) {
		...
		if (LIKELY(!result)) {
		    //执行uvc_start_streaming_bandwidth 开始数据流
...
int UVCPreview::prepare_preview(uvc_stream_ctrl_t *ctrl) {
	uvc_error_t result;
    LOGW("UVCPreview::prepare_preview");
	ENTER();
	result = uvc_get_stream_ctrl_format_size_fps(mDeviceHandle, ctrl,
		!requestMode ? UVC_FRAME_FORMAT_YUYV : UVC_FRAME_FORMAT_MJPEG,
		requestWidth, requestHeight, requestMinFps, requestMaxFps
...
void UVCPreview::do_preview(uvc_stream_ctrl_t *ctrl) {
	ENTER();
    LOGW("UVCPreview::do_preview->start_streaming");
	uvc_frame_t *frame = NULL;
	...
	{
		clearPreviewFrame();
		LOGW("UVCPreview::do_preview->capture_thread_func，创建子线程去处理回调接口");
...
/*
* 子线程处理函数帧数据流
*@参数vptr_args指向UVCP预览实例的指针
 */
// static
void *UVCPreview::capture_thread_func(void *vptr_args) {
	int result;
    LOGW("UVCPreview::capture_thread_func");
	ENTER();
	UVCPreview *preview = reinterpret_cast<UVCPreview *>(vptr_args);
	if (LIKELY(preview)) {
		JavaVM *vm = getVM();
...
void UVCPreview::do_capture_idle_loop(JNIEnv *env) {
	ENTER();
	LOGW("UVCPreview::do_capture_idle_loop");
...
void UVCPreview::do_capture_surface(JNIEnv *env) {
	ENTER();
    LOGW("UVCPreview::do_capture_surface");
	uvc_frame_t *frame = NULL;
...
void UVCPreview::do_capture_callback(JNIEnv *env, uvc_frame_t *frame) {
	ENTER();
	LOGW("UVCPreview::do_capture_callback");
	if (LIKELY(frame))
	{
		...
		if (mFrameCallbackObj)
		{
			...
			//onFrame回调给Java层
            LOGW("UVCPreview::onFrame回调给Java层");
