...
/*		Log.i(TAG, String.format("vendorId=0x%04x,productId=0x%04x,class=0x%02x,subclass=0x%02x,protocol=0x%02x",
	...
	/**
	 *从xml中读取具有默认值的布尔值（w/o异常抛出）
	 *资源布尔id也解析为布尔值
	 *若值为零，则返回false；若值为非零整数，则返回true
	 * @param context
	 * @param parser
	 * @param namespace
	 * @param name
...
/*	public void write(XmlSerializer serializer) throws IOException {
	...
	/**
	 *返回指定的类子类协议是否与此DeviceFilter匹配
	 *#isExclude或自行检查mExclude标志
	 * @param clasz
	 * @param subclass
	 * @param protocol
	 * @return
...
/*		if (mManufacturerName != null && device.getManufacturerName() == null)
	...
	/**
	 *如果mExclude与此DeviceFilter匹配且为true，则返回true
	 * @param device
	 * @return
	 */
